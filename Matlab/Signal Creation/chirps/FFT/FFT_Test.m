% Parameters
Fs = 90000;                 % Sampling frequency (Hz)
T = 0.01;                   % Duration of the tone burst (seconds)
f_tone = 8000;              % Frequency of the tone (Hz)
t = 0:1/Fs:T-1/Fs;          % Time vector

% Generate the 3000 Hz tone burst signal
...
subplot(2, 1, 1);
plot(t, signal);
title('Time-Domain Signal (3000 Hz Tone Burst)');
xlabel('Time (s)');
ylabel('Amplitude');

% Plot the frequency-domain representation (FFT)
subplot(2, 1, 2);
stem(f(1:floor(n/2)), Y_magnitude(1:floor(n/2)), 'Marker', 'none');
